...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; Modificado por Paula Villanueva Nuñez
...
(efecto prueba Covid)         ; test covid es un efecto de Covid
)

(deffacts probabilidad_variables_que_influyen
(prob zona_origen alto_riesgo 0.2)
(prob zona_origen medio_riesgo 0.4)
(prob zona_origen bajo_riesgo 0.4)
(prob inmunidad si 0.4)
(prob inmunidad no 0.6)
)

(deffacts distribucion_segun_valores_variables_que_influyen
(probcond2 Covid SI zona_origen alto_riesgo inmunidad si 0.01)
(probcond2 Covid SI zona_origen alto_riesgo inmunidad no 0.07)
(probcond2 Covid SI zona_origen medio_riesgo inmunidad si 0.001)
(probcond2 Covid SI zona_origen medio_riesgo inmunidad no 0.03)
(probcond2 Covid SI zona_origen bajo_riesgo inmunidad si 0.0001)
(probcond2 Covid SI zona_origen bajo_riesgo inmunidad no 0.005)
)

(deffacts probabilidad_efectos
(probcond tos si Covid SI 0.8)
(probcond tos si Covid NO 0.05)
(probcond fiebre alta Covid SI 0.6)
(probcond fiebre moderada Covid SI 0.3)
(probcond fiebre ausente Covid SI 0.1)
(probcond fiebre alta Covid NO 0.01)
(probcond fiebre moderada Covid NO 0.19)
(probcond fiebre ausente Covid NO 0.8)
(probcond prueba positivo Covid SI 0.9)
(probcond prueba positivo Covid NO 0.01)
)
...
(defrule preguntar_prueba
(red causal efectos)
=>
(printout t "Resultado del test covid (1=positivo 2=negativo 3=Desconocido): " )
(bind ?respuesta (read))
(if (= ?respuesta 1) then (assert (valor prueba positivo))
  else (if (= ?respuesta 2) then (assert (valor prueba negativo))
	 else (assert (valor prueba Desconocido))))
(printout t crlf)
)
